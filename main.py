...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth = np.sin(self.nodes[1]/10)
    ...
    def snapshot(self, snap_number, tau):
        k = 1
        if (snap_number % 100) < 50:
            k = -1
        elif (snap_number % 100) < 100:
            k = 1
        for i in range(len(self.nodes[0])):
            if self.nodes[1][i] > 0:
                # self.velocity[0][i] = 8 * math.sin(self.nodes[1][i] / 10)
                self.velocity[0][i] = (k * self.nodes[1][i] / 20)
            if self.nodes[1][i] > 0:
                self.velocity[2][i] = (k * self.nodes[1][i] / 40)


        self.smth = np.sin(self.nodes[1] / 5 + tau * n * 2)
